...
Feature:
  ...
  Scenario: upload files to DCAS and create index file on Sidetree that references them
    ...
    Given variable "fileHandlerConfig" is assigned the JSON value '{"Handlers":[{"Description":"For verifiable credential schemas","BasePath":"/schema","ChaincodeName":"files","Collection":"consortium","IndexNamespace":"file:idx","IndexDocID":"${schemaIndexID}"},{"Description":"Consortium .wellknown files","BasePath":"/.well-known/did-bloc","ChaincodeName":"files","Collection":"consortium","IndexNamespace":"file:idx","IndexDocID":"${wellKnownIndexID}"}]}'
