...
defineSupportCode(function ({And, But, Given, Then, When}) {
    ...
    And(/^anchor address is parsed from anchor string "([^"]*)" and saved to variable "([^"]*)"$/, function (arg1, arg2, callback) {
        callback.pending();
    });
    And(/^response is decompressed using "([^"]*)"$/, function (arg1, arg2, callback) {
        callback.pending();
    });
    And(/^anchor address is parsed from transaction info "([^"]*)" and saved to variable "([^"]*)"$/, function (arg1, arg2, callback) {
        callback.pending();
    });
});
